...
from AdaptiveModel import StaticModel
from AdaptivePolicy import Policy
from AdaptiveUtil import createStochasticGraph

if __name__ == "__main__":
    seed = 89720123

    # read parameter file and initialize variables
    ...
    # create a random graph and compute V_0
    g, V_t, origin_node, target_node, dist = createStochasticGraph(params)
    params.update({"start_node": origin_node, "target_node": target_node})

    # create the model, given the above policy
    M = StaticModel(state_names, decision_names, params, g, V_t)
    ...
    for theta in theta_list:
        ...
        for ite in list(range(model.init_args["nIterations"])):
            model.obj = 0
            # Reset current node to starting node
            model.state = model.build_state(model.init_state)

            print(
                "\tTheta {}  - Iteration {} - Stepsize {:.2f} - InitState {} - Vbar {:.2f}".format(
                    model.theta_step,
                    model.n,
                    model.alpha(),
                    model.state.CurrentNode,
                    model.V_t[model.state.CurrentNode],
                )
            )

            step = 1
            while model.state.CurrentNode != target_node:
                # calling policy and choosing a decision
                decision, vhat = P.make_decision(model)
                x = model.build_decision({"NextNode": decision})

                # print(
                #    "\t\t Theta {}  - Iteration {} - Step {} - Current State {} - vbar = {:.2f}".format(
                #        model.theta_step,
                #        model.n,
                #        step,
                #        model.state.CurrentNode,
                ...
                # model.print_State()
                # print(
                #    "\t\tDecision={}, vhat {:.2f} and new vbar for current state {:.2f}".format(
                #        x[0], vhat, model.V_t[model.state.CurrentNode]
                #    )
                # )
            ...
            print(
                "Finishing Theta {} and Iteration {} with {} steps. Total cost: {} \n".format(
                    model.theta_step, model.n, step - 1, model.obj
                )
            )
            model.n += 1
            obj_along_iterations[theta].append(model.obj)
            vbar_along_iterations[theta].append(model.V_t[origin_node])

    # Plotting the results
    fig1, axsubs = plt.subplots(1, 2)
    fig1.suptitle(
        "Comparison of theta^step - stepsize type {} - origin {}, target {}, dist {} ".format(
            M.init_args["stepsize_rule"], origin_node, target_node, dist
        )
    )
    ...
    totals = [np.array(obj_along_iterations[theta]).mean() for theta in theta_list]
    print("Totals ", totals)
    ...
    for theta, t in zip(theta_list, nThetas):
        ...
        axsubs[1].plot(
            Iterations,
            vbar_along_iterations[theta],
            "{}o-".format(color_list[t]),
            label="{}".format(theta),
        )
