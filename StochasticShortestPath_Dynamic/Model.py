...
class StaticModel:
    ...
    def update_estimated_costs(self):
        for k in range(self.G.vertexCount):
            for l in self.G.neighbors[k]:
                ...
                if alpha < 1:
                    self.estimated_costs[k][l] = (1 - alpha) * self.estimated_costs[k][
                        l
                    ] + alpha * m_hat
                else:
    ...
    def sample_from_uniform(self, fromNode, toNode):
        ...
        deviation = (
            self.prng.uniform(-spread, spread) * self.G.meanCosts[fromNode][toNode]
        )
    ...
    def runTrials(self, nbTrials, deadline):
        ...
        for i in range(nbTrials):
            ...
            print(
                "End of Theta {:.2f}, Iteration {}. Cost: {:.2f}, Lateness: {:.2f} and number of steps {}".format(
                    self.theta, self.n, cost, math.sqrt(latenessSquared), self.time - 1
                )
            )
