...
class GraphGenerator:
    ...
    def __init__(self, params):
        ...
        # The start and end node will change based on the network graph that is going to be constructed -
        # we are going to select the pair with the longest shortest path
        self.start_node = 0
        self.end_node = 0
        self.steps = 0
        self.vertexCount = 1
            ...
            for i in nodesPerLevel[level]:
                ...
                edge_set = list(
                    self.prng.choice(
                        ...
                        min(3, len(nodesPerLevel[level + 1])),
                        replace=False,
                    )
                )
                for j in edge_set:
    ...
    def get_deadline(self):
        return (
            self.init_args["costMin"]
            + (self.init_args["costMax"] - self.init_args["costMin"])
            * self.init_args["deadlinePerc"]
        ) * (self.steps)
    ...
    def get_avg_cost_paths(self, shouldPrintPaths=False):
        ...
        for path in self.mPathsList:
            ...
            for n in range(nSteps - 1):
                ...
                # pathString += edge
            pathString += " - {} steps and {:.2f} total mean cost".format(
                nSteps, totalCost
            )
    ...
    def construct_network_objects(self, G, filename, start_node, end_node):
        ...
        for fromNode in range(size):
            ...
            for toNode in G.neighbors(fromNode):
                ...
                self.meanCosts[fromNode][toNode] = self.prng.uniform(
                    self.init_args["costMin"], self.init_args["costMax"]
                )
                self.spreads[fromNode][toNode] = self.prng.uniform(
                    0, self.init_args["maxSpreadPerc"]
                )
                ...
                record = (
                    fromNode,
                    toNode,
                    self.meanCosts[fromNode][toNode],
                    self.spreads[fromNode][toNode],
                    size,
                )
        ...
        self.mPathsList = list(
            nx.all_simple_paths(G, source=self.start_node, target=self.end_node)
        )
    ...
    def createNetworkChance(self):
        ...
        while done == 0:
            ...
            print(
                "Iteration {}, Source {}, Dest {}, Length {}, number of paths {}".format(
                    nbIterations, mSource, mDest, maxLength, mPaths
                )
            )

            if (
                maxLength > self.init_args["lengthThreshold"]
                and mPaths > self.init_args["numberPathsThreshold"]
            ):
