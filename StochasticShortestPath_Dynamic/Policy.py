...
class LookaheadPolicy:
    ...
    def get_decision(self, METRIC):
        ...
        decisions = [
            ([0] * self.model.G.vertexCount) for row in range(self.model.G.Horizon + 1)
        ]
            ...
            for k in range(self.model.G.vertexCount):
                ...
                for l in self.model.G.neighbors[k]:
                    if METRIC == "PERCENTILE":
                        ...
                        if minVal >= V[lookAheadTime + 1][l] + self.use_percentile_val(
                            self.model.theta, spread, mean
                        ):
                            argMin = l
                            minVal = V[lookAheadTime + 1][l] + self.use_percentile_val(
                                self.model.theta, spread, mean
                            )
                    else:
