...
def process_raw_price_data(file, params):
    ...
    print(
        "Processing raw price data. Constructing price change list and cdf using {}".format(
            DISC_TYPE
        )
    )
    tS = time.time()
    ...
    data_selection["Price_Change"] = (
        data_selection["PJM_RT_LMP"] - data_selection["Price_Shift"]
    )
    ...
    print(
        "Min price change {:.2f} and Max price change {:.2f}".format(
    ...
    if DISC_TYPE == "FROM_CUM":
        ...
        discrete_price_change = np.arange(
            min_price_change, max_price_change, price_change_increment
        )
        discrete_price_change_list = list(
            np.append(discrete_price_change, max_price_change)
        )
    ...
    discrete_price_change_pdf = discrete_price_change_cdf - shift(
        discrete_price_change_cdf, 1, cval=0
    )
    ...
    print(
        "Finishing processing raw price data in {:.2f} secs. Expected price change is {:.2f}. Hist_price len is {}".format(
            time.time() - tS, mean_price_change, len(hist_price)
        )
    )
...
if __name__ == "__main__":
    ...
    initial_state = {
        "price": exog_params["hist_price"][0],
    ...
    if params["Algorithm"] == "GridSearch":
        ...
        contribution_values_dict = P.perform_grid_search(
            params, grid_search_theta_values[0]
        )
    ...
    if params["Algorithm"] == "BackwardDP":
        ...
        discrete_energy = np.array([0.0, 1.0])
        ...
        for inc in params["price_disc_list"]:
            ...
            test_2D = BDP(
                discrete_prices,
                discrete_energy,
                exog_params["price_changes_sorted"],
                exog_params["discrete_price_change_list"],
            ...
            print(
                "Contribution using BackwardDP 2D is {:.2f}. Finished in {:.2f}s".format(
                    contribution, time.time() - tS
                )
            )
            ...
            if params["run3D"]:
                ...
                print(
                    "Contribution using BackwardDP 3D is {:.2f}. Finished in {:.2f}s".format(
                        contribution, time.time() - tS
                    )
                )
