...
class EnergyStoragePolicy:
    ...
    def bellman_policy(self, time, state, bellman_model):
        ...
        for d in self.model.possible_decisions:
            ...
            for w in bellman_model.discrete_price_changes:
                f = (
                    bellman_model.f_p[w_index]
                    if w_index == 0
                    else bellman_model.f_p[w_index] - bellman_model.f_p[w_index - 1]
                )
                next_state = bellman_model.state_transition(state, x, w)
                next_v = (
                    bellman_model.values_dict[time + 1][next_state]
                    if time < bellman_model.time
    ...
    def run_policy(self, policy_info, policy, stop_time):
        ...
        print(
            "Energy traded - Sell: {:.2f} - Buy: {:.2f} - Hold % : {:.2f}".format(
                nTrades["sell"],
                nTrades["buy"],
                nTrades["hold"] / model_copy.init_args["T"],
            )
        )
        print("Sell times and prices ")
        for i in range(len(sell_list)):
            print(
                "t = {:.2f} and price = {:.2f}".format(sell_list[i][0], sell_list[i][1])
            )
        print("Buy times and prices ")
        for i in range(len(buy_list)):
            print(
                "t = {:.2f} and price = {:.2f}".format(buy_list[i][0], buy_list[i][1])
            )
    ...
    def perform_grid_search(self, params, theta_values):
        ...
        for theta in theta_values:
            ...
            if theta[0] >= theta[1]:
                ...
                contribution = self.run_policy(
                    theta, "buy_low_sell_high_policy", params["T"]
                )
                contribution_values_dict[theta] = contribution
                best_theta = max(
                    contribution_values_dict, key=contribution_values_dict.get
                )
                print(
                    "Finishing theta {} with contribution {:.2f}. Best theta so far {}. Best contribution {:.2f}".format(
                        theta,
                        contribution,
                        best_theta,
                        contribution_values_dict[best_theta],
                    )
                )
    ...
    def grid_search_theta_values(self, params):
        ...
        """
        theta_buy_values = np.arange(
            params["theta_buy_min"], params["theta_buy_max"], params["theta_inc"]
        )
        theta_sell_values = np.arange(
            params["theta_sell_min"], params["theta_sell_max"], params["theta_inc"]
        )
    ...
    def plot_heat_map(self, contribution_dict, theta_buy_values, theta_sell_values):
        ...
        contribution_values = [
            contribution_dict[(theta_buy, theta_sell)]
            for theta_sell in theta_sell_values
            for theta_buy in theta_buy_values
        ]
