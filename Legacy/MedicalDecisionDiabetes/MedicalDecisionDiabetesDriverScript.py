...
if __name__ == "__main__":
    ...
    theta_range_1 = np.arange(
        additional_params.loc["theta_start", 0],
        additional_params.loc["theta_end", 0] + additional_params.loc["increment", 0],
    ...
    labelsOutputPath = (
        ["Policy", "Truth_type", "Theta", "Sample_path"]
        + mu_star_labels
        + ["Best_Treatment", "n"]
        + mu_labels
        + sigma_labels
        + N_labels
        + ["Decision", "W", "CumReward", "isBest"]
    )
    ...
    best_treat = {
        (p, theta): [] for p in policy_list for theta in theta_range_1
    }  # one list for each (p,theta) pair - each list is along the sample paths
    best_treat_count_list = {
        (p, theta): [] for p in policy_list for theta in theta_range_1
    }  # one list for each (p,theta) pair - the list is along the sample paths - each element of the list is accumulated during the experiments
    best_treat_Counter_hist = {
        (p, theta): [] for p in policy_list for theta in theta_range_1
    }
    best_treat_Chosen_hist = {
        (p, theta): [] for p in policy_list for theta in theta_range_1
    }

    # data structures to accumulate the decisions
    decision_Given_Best_Treat_list = {
        (p, theta, d): []
        for p in policy_list
        for theta in theta_range_1
        for d in x_names
    }
    decision_Given_Best_Treat_Counter = {
        (p, theta, d): []
        for p in policy_list
        for theta in theta_range_1
        for d in x_names
    ...
    for policy_chosen in policy_list:
        ...
        for theta in theta_range_1:
            ...
            F_hat = []
            last_state_dict = {x: [0.0, 0.0, 0] for x in x_names}
            states_avg = {}
            ...
            for l in range(1, L + 1):
                ...
                record_sample_l = (
                    [policy_chosen, Model.truth_type, theta, l]
                    + mu_output
                    + [best_treatment]
                )
                ...
                for n in range(t_stop):
                    ...
                    state_sigma = [
                        1 / math.sqrt(getattr(model_copy.state, x)[1]) for x in x_names
                    ]
                    ...
                    record_sample_t = (
                        ...
                        + [
                            ...
                            exog_info["reduction"],
                            model_copy.obj,
                            decision == best_treatment and 1 or 0,
                        ]
                    )
                ...
                last_state_dict.update(
                    {
                        x: [
                            last_state_dict[x][0] + getattr(model_copy.state, x)[0],
                            last_state_dict[x][1] + getattr(model_copy.state, x)[1],
                            last_state_dict[x][2] + getattr(model_copy.state, x)[2],
                        ]
                        for x in x_names
                    }
                )
                ...
                decision_Given_Best_Treat_list[
                    (policy_chosen, theta, best_treatment)
                ] += decision_list
            ...
            print(
                "Finishing policy = {}, Truth_type {} and theta = {}. F_bar_mean = {:.3f} and F_bar_std = {:.3f}".format(
                    policy_chosen,
                    Model.truth_type,
                    theta,
                    F_hat_mean,
            ...
            states_avg = {
                x: [
                    last_state_dict[x][0] / L,
                    last_state_dict[x][1] / L,
                    last_state_dict[x][2] / L,
                ]
                for x in x_names
            }
            ...
            for x in x_names:
                print(
                    "Treatment {}: m_bar {:.2f}, beta_bar {:.2f} and N {}".format(
                        x, states_avg[x][0], states_avg[x][1], states_avg[x][2]
                    )
                )
            ...
            hist, bin_edges = np.histogram(
                np.array(best_treat_count_list[(policy_chosen, theta)]), t_stop
            )
            ...
            decision_ALL_Counter[(policy_chosen, theta)] = normalizeCounter(
                Counter(decision_ALL_list[(policy_chosen, theta)])
            )
            ...
            decision_Given_Best_Treat_dict = {
                x: dict(
                    normalizeCounter(
                        Counter(
                            decision_Given_Best_Treat_list[(policy_chosen, theta, x)]
                        )
                    )
                )
                for x in Model.x_names
            }
        ...
        print(
            "Ending policy {}. Elapsed time {} secs\n\n\n".format(
                policy_chosen, policy_end - policy_start
            )
        )
    ...
    if print_excel_file:
        ...
        print(
            "Finished printing the excel file. Elapsed time {}".format(
                print_end_time - print_init_time
            )
        )
    ...
    for policy_chosen, p in zip(policy_list, nPolicies):
        axsubs[0].plot(
            theta_range_1,
            theta_obj[policy_chosen],
            "{}o-".format(color_list[p]),
            label="{}".format(policy_chosen),
        )
