...
class MedicalDecisionDiabetesModel:
    ...
    def __init__(
        ...
        self.init_state = {
            x: [S0.loc[x, "mu_0"], Beta(S0.loc[x, "sigma_0"]), 0] for x in x_names
        }
        ...
        self.mu = (
            {}
        )  # updated using "exog_info_sample_mu" at the beginning of each sample path
        ...
        if self.truth_type == "fixed_uniform":
            self.truth_params_dict = {
                x: [
                    S0.loc[x, "mu_fixed"],
                    S0.loc[x, "fixed_uniform_a"],
                    S0.loc[x, "fixed_uniform_b"],
                ]
            ...
            self.truth_params_dict = {
                x: [
                    S0.loc[x, "mu_0"],
                    S0.loc[x, "prior_mult_a"],
                    S0.loc[x, "prior_mult_b"],
                ]
                for x in self.x_names
            }
        else:
            self.truth_params_dict = {
                x: [S0.loc[x, "mu_truth"], S0.loc[x, "sigma_truth"], 0]
                for x in self.x_names
            }
    ...
    def printState(self):
        ...
        for x in self.x_names:
            print(
                "Treatment {}: mu {:.2f}, sigma {:.2f} and N {}".format(
                    x,
                    getattr(self.state, x)[0],
                    1 / math.sqrt(getattr(self.state, x)[1]),
                    getattr(self.state, x)[2],
                )
            )
        print("\n\n")
    ...
    def printTruth(self):
        print(
            "Model truth_type {}. Meaurement noise sigma_W {} ".format(
                self.truth_type, self.sigma_w
            )
        )
        for x in self.x_names:
            print(
                "Treatment {}: par1 {:.2f}, par2 {:.2f} and par3 {}".format(
                    x,
                    self.truth_params_dict[x][0],
                    self.truth_params_dict[x][1],
                    self.truth_params_dict[x][2],
                )
            )
        print("\n\n")
    ...
    def exog_info_sample_mu(self):
        if self.truth_type == "known":
            ...
            self.mu = {
                ...
                + self.prng.uniform(
                    self.truth_params_dict[x][1], self.truth_params_dict[x][2]
                )
                for x in self.x_names
            }
        elif self.truth_type == "prior_uniform":
            ...
            self.mu = {
                x: self.prng.normal(
                    self.truth_params_dict[x][0], self.truth_params_dict[x][1]
                )
                for x in self.x_names
            }
    ...
    def transition_fn(self, decision, exog_info):
        ...
        mu_empirical = (
            (getattr(self.state, decision))[1] * (getattr(self.state, decision))[0]
            + exog_info["beta_W"] * exog_info["reduction"]
        ) / beta
        N_x = (
            getattr(self.state, decision)[2] + 1
        )  # count of no. times drug x was given.
    ...
    def step(self, decision):
        ...
        current_state = {
            key: exog_info[decision] if key == decision else getattr(self.state, key)
